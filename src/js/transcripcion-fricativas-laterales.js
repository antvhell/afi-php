...
window.onload = function () {
  var letras = ["l", "a", "e", "n", "v", "ʈ", "ʔ", "ʤ", "ʦ", "o", "h", "ʧ", "u", "ɬ", "ɢ", "ɖ", "i", "k", "m", "p"];
  ...
  function cargaEventos(letras, evento, tipo, funcion, activa, clase) {
    return letras
      .map((letra) => {
        return `<input type="${tipo}" value="${letra}" id="${letra}" ${evento}="${funcion}" ${activa} class="${clase}">\n`;
      })
      .join("");
  }
};
...
function finJuego(cadena) {
  ...
  if (palabras[indice] === "aɬik") {
    document.getElementById("glosa").innerHTML = `'hoja de papel' en tepehua`;
